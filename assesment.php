        		...
        		*/
                ...
                $(function(){
                    ( function () {
                        ...
                        Morris.Bar({
                            element: "'.$data_digestate["gender"][0].'_'.$current_position.'" ,
                            data: [
                            { y: "student", a: '.$student.'  },
                            { y: "faculty", a: '.$faculty.' },
                            { y: "staff", a: '.$staff.'  }
...
switch ( $conveyed_data ):
	...
	case "get_female_data":
		...
		echo $rec->generate_display( $rec->digest( $rec->female_data ) );
	break;
	
	case "get_female_bar":
		echo $rec->generate_bar( $rec->digest( $rec->female_data ) );
	break;
	
	case "get_male_data":
		//echo $rec->digest( $rec->male_data );
		echo $rec->generate_display( $rec->digest( $rec->male_data ) );
	break;
	
	case "get_male_bar":
		echo $rec->generate_bar( $rec->digest( $rec->male_data ) );
	break;
	...
	case "last_three":
		echo json_encode( $rec->last_three_data() );		
	break;
